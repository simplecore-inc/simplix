...
헤더로 인증하는 애플리케이션은 `EventSource`가 헤더를 보내지 못하므로 연결 티켓을 사용합니다. 자세한 내용은 [JavaScript 클라이언트 가이드](./client-javascript-guide.md#연결-인증)를 참고하세요.
...
- [SSE 분산 모드 (Redis) 튜토리얼](./tutorial-sse-distributed-redis.md) - 다중 인스턴스 환경 구성
- [Admin API 가이드](./admin-api-guide.md) - 세션/스케줄러 관리
- [모니터링 가이드](./monitoring-guide.md) - 메트릭 및 헬스 체크
