...
public class EventConfigurationValidator {
    ...
    private void validateMode() {
        ...
        if (mode == null || mode.trim().isEmpty()) {
            log.warn("No event mode specified, defaulting to 'local'");
    ...
    private void validateRedisConfiguration() {
        ...
        String channelPrefix = environment.getProperty("simplix.events.redis.channel-prefix");
        if (channelPrefix == null || channelPrefix.trim().isEmpty()) {
            log.warn("Redis channel prefix not configured, using default 'simplix:events:'");
        }
    }
    ...
    private void logConfigurationSummary() {
        ...
        switch (mode) {
            case "redis":
                ...
                log.info("Redis Persistent Storage: {}",
                    environment.getProperty("simplix.events.redis.persistent.enabled", "false"));
                break;
            case "kafka":
