...
해석 순서는 등록된 `ExcelLabelResolver` → 애플리케이션 `MessageSource` → `@ExcelColumn`에 적힌 문자열입니다. 번역이 없어도 파일은 만들어지며, 키가 그대로 남습니다. 자세한 내용은 [내보내기 가이드](docs/ko/export-guide.md)에 있습니다.
...
- [Export Guide (내보내기 가이드)](docs/ko/export-guide.md)
- [Import Guide (가져오기 가이드)](docs/ko/import-guide.md)
