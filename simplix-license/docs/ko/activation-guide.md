...
빌드가 유출로 표시된 검증 키를 담고 있고 저장소에 아무것도 없으면, 파일을 읽기 전에 천장이 "보유한 것 없음"으로 고정됩니다. 파일을 먼저 읽으면 손으로 넣은 토큰이 기준이 되어, 새 배포본에 위조 토큰을 먹이는 경로가 그대로 열립니다. 자세한 내용은 [유출 키 천장](./overview.md#compromiseceiling)에 있습니다.
...
4. 등록은 됐는데 상태가 `SIGNING_KEY_COMPROMISED`라면 [강제 적용 가이드](./enforcement-guide.md#signing_key_compromised로-차단됩니다)를 봅니다.
