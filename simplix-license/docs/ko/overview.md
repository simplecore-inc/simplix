...
| 검증 공개키 위치가 고정 | 배포본이 스스로 서명한 토큰을 검증하지 못하게 함 |
...
애플리케이션이 반드시 등록해야 하는 빈은 `ProductIdentity`와 `VerificationKeyIdentity` 둘입니다. 없으면 컨텍스트가 기동하지 않습니다.
...
DB 저장소는 Spring Data 리포지토리 대신 `EntityManager`로 질의합니다. 프레임워크 모듈에서 `@EnableJpaRepositories`를 선언하면 애플리케이션의 리포지토리 자동 구성이 꺼지기 때문입니다.
