...
요청 단위로 판정을 적용합니다. 상태별 동작은 [README](ko/README.md#license-status)의 표를 따르며, 차단 상태에서도 인증·라이선스 등록·설치 마법사 경로는 열려 있습니다. 이 예외가 없으면 라이선스가 없는 배포본은 로그인할 수 없어 라이선스를 등록할 수도 없게 됩니다.
...
전체 목록은 [설정 레퍼런스](ko/license/configuration.md)를 참고하세요.
...
- [활성화 가이드](ko/license/activation-guide.md) - 온라인·오프라인 등록 절차
