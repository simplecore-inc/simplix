...
```
refresh-token-lifetime = 3시간
로테이션 주기 = 매주 일요일 (7일)
키 보존 시간 = 3시간 + 1시간(buffer) = 4시간

Timeline:
┌──────────────────────────────────────────────────────────────┐
│ Day 0: Key v1 생성 (active)                                  │
│ Day 7: Key v2 생성 (active), v1은 4시간 후 삭제 가능           │
│ Day 14: Key v3 생성 (active), v2는 4시간 후 삭제 가능          │
└──────────────────────────────────────────────────────────────┘

토큰 유효기간이 짧아도 로테이션 주기는 길게 유지 가능!
이전 키는 토큰 만료 후 빠르게 삭제되어 저장 공간 절약
```
...
│                    │  jwe-v1702245678901 │ (inactive, 만료 전)   │
│                    └─────────────────────┘                      │
└───────────────────────────┬─────────────────────────────────────┘
                            │
        ┌───────────────────┼───────────────────┐
...
```
T+0초:  서버A가 새 키 v2 생성 & 활성화
T+0초:  서버A가 v2로 토큰 암호화 (kid=v2) → 사용자에게 발급
T+1초:  사용자가 서버B에 요청 (서버B 캐시: 아직 v1만)
T+1초:  서버B가 토큰의 kid=v2 확인
T+1초:  캐시 미스 → loadKeyOnDemand("v2") → DB에서 로드 ✔
T+1초:  복호화 성공! ✔
```
...
- [simplix-encryption 문서](../../simplix-encryption/docs/ko/)
